...
const versionData = ipcRenderer.sendSync(
  "getApi",
  "https://ddragon.leagueoflegends.com/api/versions.json"
).body[0];
const currentSummoner = ipcRenderer.sendSync(
  "get",
...
var versionCDN = ipcRenderer.sendSync(
  "getApi",
  "https://raw.githubusercontent.com/Tortudereli/TortudereliTools/main/status.json"
)["body"];

if (versionCDN["version"] > appVersion) {
  document.getElementById("currentVersion").style.color = "red";
}

document
  .getElementById("goProfile")
  .setAttribute("href", `${dirname}\\src\\html\\profile\\index.html`);
document
  .getElementById("goInventory")
  .setAttribute("href", `${dirname}\\src\\html\\inventory\\index.html`);
document
  .getElementById("goTools")
  .setAttribute("href", `${dirname}\\src\\html\\tools\\index.html`);

let walletData;
const donateListData = ipcRenderer.sendSync(
  "getApi",
  "https://raw.githubusercontent.com/Tortudereli/TortudereliToolsCDN/main/donate/donateData.json"
).body;
donateListData.forEach((element) => {
  document.getElementById(
    "selectBox"
  ).innerHTML += `<option value="${element.short}">${element.name}</option>`;
});
  ...
  donateListData.forEach((element) => {
    if (element.short == document.getElementById("selectBox").value) {
      if (typeof element["networks"] !== "undefined") {
        ...
        walletData = ipcRenderer.sendSync(
          "getApi",
          "https://raw.githubusercontent.com/Tortudereli/TortudereliToolsCDN/main/donate/walletAdress.json"
        ).body["walletAddress"][element.short];
        document
          .getElementById("donateQrImg")
          .setAttribute("src", walletData.qrURL);
...
function updateAddressData() {
  ...
  document
    .getElementById("donateQrImg")
    .setAttribute("src", walletData[networkType].qrURL);
  document.getElementById("donateDesc").innerText =
    walletData[networkType].address;
  if (networkType == "BEP2") {
    document.getElementById("donateDesc2").innerText =
