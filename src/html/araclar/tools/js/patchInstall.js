...
var infoSound = new Audio("../../../sounds/sfx-ps-ui-nav-button-click.ogg");
...
var patchData = ipcRenderer.sendSync(
  "get",
...
patchData["body"]["supported_game_releases"].forEach((element) => {
  $("#selectPatch").append(
    `<option value="${element["download"]["url"]}">${element[
      "artifact_id"
    ].substring(0, element["artifact_id"].indexOf("+"))}</option>`
  );
});

var activeSound = new Audio("../../../sounds/sfx-gameselect-button-confirm-click.ogg");
$("#submitPatchButton").click(() => {
  ...
  if ($("#typeSelect").prop("checked") == true) {
    if ($("#selectPatch").val() == 0) {
      ...
      json = {
        url: `/lol-patch/v1/game-patch-url?url=${encodeURIComponent(
          $("#selectPatch").val()
        )}`,
        json: "",
      };
  ...
  if (status["status"] == 201) {
    ...
    if (status1["status"] == 204) {
      activeSound.play();
      $("#submitPatchButton").attr("disabled", true);
      alert("Başarılı! Yama yüklenmeye başladı!.");
    } else {
